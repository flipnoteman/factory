...
impl<T> AVec<T> {
        ...
        T: Copy
    {
        ...
        unsafe {
            let dest_ptr = self.pointer.as_ptr().add(self.length);
...
//     pub unsafe fn append_bytes(&mut self, other: *const u8, size: usize) {
//         unsafe {
//             core::ptr::copy_nonoverlapping(other, self.pointer.as_ptr() as *mut u8, size);
//         }
//     }
...
mod tests {
    ...
    fn extend_from_slice_test() {
        ...
        t.extend_from_slice(arr.as_slice());
