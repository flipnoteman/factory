...
use core::{alloc::Layout, ffi::c_void, fmt::Debug, ptr::{null_mut, slice_from_raw_parts_mut}, clone::Clone };
use psp::sys::{IoOpenFlags, SceIoStat, sceIoClose, sceIoGetstat, sceIoRead};
...
impl Asset for BMP {
    ...
    fn load(&mut self) -> Result<(), &str> {
        unsafe {
            ...
            let row_size = if self.bih.bits == 32 {
                self.bih.width * 4
            } else {
                self.bih.width * 3
            };
            ...
            self.handle = Some(
                alloc(Layout::from_size_align(data_size as usize, 16).unwrap()) as *mut c_void,
            );
            ...
            if sceIoClose(self.file_descriptor) < 0 {
                dealloc(
                    handle,
                    Layout::from_size_align(data_size as usize, 16).unwrap(),
                );
                dealloc(tmp_handle as *mut u8, Layout::array::<u8>(size).unwrap());
                return Err("Failed to close file.");
            };
            ...
            for y in 0..self.bih.height {
                // Starting row
                let src_row = if self.bih.height > 0 {
                    self.bih.height - y - 1
                } else {
                    y
                };
